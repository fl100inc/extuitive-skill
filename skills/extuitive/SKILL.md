---
name: extuitive
description: Work with Extuitive, where each workspace is one Facebook ads account. Check that Extuitive is connected and signed in, choose which ad account to work in, upload images and videos in bulk, check how an upload is going and which files were accepted, read what uploaded files contain or find uploads like them, collect files from a client through a one-time upload link, register uploads with the Meta ad account, build or look up a Meta campaign, ad set, creative and ad, connect or repair a Meta connection, or set Extuitive up from scratch. Use when asked to check the Extuitive connection or whether it is set up, to upload or import creative, ad images, ad videos or a folder of assets into Extuitive, to check an upload or say what is in it or what a video says, to find uploads that look alike or match a description, to send someone an upload link, to publish uploads to Meta, to create or list campaigns, ad sets, ads, pages or pixels, to switch ad accounts, or when Extuitive tools are missing or refusing.
